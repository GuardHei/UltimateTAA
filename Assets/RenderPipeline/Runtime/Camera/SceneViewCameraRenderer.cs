...
public class SceneViewCameraRenderer : GameCameraRenderer {
	...
	public SceneViewCameraRenderer(Camera camera) : base(camera) {
		_rendererDesc = "Render Scene View (" + camera + ")";
	...
	public void DrawUnsupportedShaders() {
		...
		for (var i = 0; i < ShaderTagManager.LEGACY_SHADER_TAGS.Length; i++) drawSettings.SetShaderPassName(i, ShaderTagManager.LEGACY_SHADER_TAGS[i]);
		
		var filterSettings = new FilteringSettings(ShaderTagManager.ALL_QUEUE);
