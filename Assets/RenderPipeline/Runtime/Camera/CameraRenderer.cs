...
public unsafe abstract class CameraRenderer : IDisposable {
    ...
    public void SetRenderTarget(RTHandle colorBuffer, int mipLevel, CubemapFace cubemapFace, int depthSlice, bool clear = false) {
    ...
    public void SetRenderTarget(RTHandle colorBuffer, RTHandle depthBuffer, int mipLevel, CubemapFace cubemapFace, bool clear = false) {
        _cmd.SetRenderTarget(colorBuffer, depthBuffer, mipLevel, cubemapFace, 0);
        CoreUtils.SetViewport(_cmd, colorBuffer);
        if (clear) ClearRenderTarget(RTClearFlags.All);
    }
    
    public void SetRenderTarget(RTHandle colorBuffer, RTHandle depthBuffer, int mipLevel, CubemapFace cubemapFace, int depthSlice, bool clear = false) {
        _cmd.SetRenderTarget(colorBuffer, depthBuffer, mipLevel, cubemapFace, depthSlice);
        CoreUtils.SetViewport(_cmd, colorBuffer);
        if (clear) ClearRenderTarget(RTClearFlags.All);
    }

    public void SetRenderTarget(RTHandle colorBuffer, RenderBufferLoadAction colorLoad, RenderBufferStoreAction colorStore, RTHandle depthBuffer, RenderBufferLoadAction depthLoad, RenderBufferStoreAction depthStore, bool clear = false) {
    ...
    public void SetRenderTarget(RTHandle[] colorBuffers, RTHandle depthBuffer, int mipLevel, CubemapFace cubemapFace, bool clear = false) {
        _cmd.SetRenderTarget(ARPUtils.RTHandlesToRTIs(colorBuffers), depthBuffer, mipLevel, cubemapFace, 0);
        CoreUtils.SetViewport(_cmd, colorBuffers[0]);
    }

    public void SetRenderTarget(RTHandle[] colorBuffers, RTHandle depthBuffer, int mipLevel, CubemapFace cubemapFace, int depthSlice, bool clear = false) {
        _cmd.SetRenderTarget(ARPUtils.RTHandlesToRTIs(colorBuffers), depthBuffer, mipLevel, cubemapFace, depthSlice);
        CoreUtils.SetViewport(_cmd, colorBuffers[0]);
    }
    
    public void ClearRenderTarget(RTClearFlags flags) => ClearRenderTarget(flags, Color.black);

    public void ClearRenderTarget(RTClearFlags flags, Color color, float depth = 1f, uint stencil = 0) => _cmd.ClearRenderTarget(flags, color, depth, stencil);

    public void ClearRenderTarget(bool clearColor, bool clearDepth) => ClearRenderTarget(clearColor, clearDepth, Color.black);
    
    public void ClearRenderTarget(bool clearColor, bool clearDepth, Color color, float depth = 1f) =>  _cmd.ClearRenderTarget(clearDepth, clearColor, color, depth);
